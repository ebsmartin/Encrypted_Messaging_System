...
public class Contact {
    ...
    // if something goes hay wire, edit the PhoneNumber number if then statement to allow any number of digits
    // TODO STUDENT - COMPLETE
    public String cleanNumber(String number) {
        StringBuilder str = new StringBuilder();  // StringBuilder is a built in method
        for(int i = 0; i < number.length(); i++) {
        	char tempChar = number.charAt(i);
        	
        	if(!Character.isWhitespace(tempChar) == true) {
    ...
    /** returns if the user has an ename - which is the first half of their email **/
